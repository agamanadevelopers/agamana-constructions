...
export interface GroupEcosystemSectionCms {
  eyebrow?: string;
  sectionTitle?: string;
...
export default function GroupEcosystem({
  ...
  section?: GroupEcosystemSectionCms | null;
  ecosystem?: EcosystemItemCms[] | null;
}) {
  const data = ecosystem?.length ? ecosystem : staticEcosystem;
  const eyebrow = section?.eyebrow ?? 'The Agamana Group';
  const sectionTitle = section?.sectionTitle ?? 'More Than Construction.';
  const sectionSubtitle = section?.sectionSubtitle ?? 'Agamana Constructions is part of the broader Agamana Group, bringing together complementary capabilities across the real estate, development, design and construction ecosystem.';
  const ctaText = section?.ctaText ?? 'Explore the Agamana Group';
  const tagline = section?.tagline ?? 'Different capabilities. One integrated ecosystem.';
  const featureImage = section?.featureImage ?? images.groupFeature;
  ...
  return (
    <section id="about" className="bg-cream py-14 sm:py-[70px]">
      <div className="container-page">
        ...
        <p className="mt-9 text-center text-base font-medium text-brand">{tagline}</p>
      </div>
    </section>
  );
}
